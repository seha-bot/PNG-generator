...
void banana(v3 *value, int x, int y)
{
    ...
    *value = vec(value->x, value->y, circle(x-480, y-250, 40) == 255 ? 255 : value->z);
    ...
    if(value->x == 0 && value->y == 0 && value->z == 0) *value = vec(0, 0, 10);
    ...
    //Background
    if(value->x == 0 && value->y == 0 && value->z == 10) *value = vec(100, 100, 100);

    //Shadows
    int shadow = ellipse(x - halfH - 10, y - halfH + 270, 350, 30);
    int s;
    s = length(x - halfH - 10, y - halfH + 270) - 2;
    s = ((x - halfH - 10)/14.0f) * ((x - halfH - 10)/14.0f) + ((y - halfH + 270)/1.5f) * ((y - halfH + 270)/1.5f) - 1;
    if(s > 100) s = 100;
